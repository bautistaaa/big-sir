...
const AlbumFeedCard = ({ listId }: Props) => {
  ...
  const listDetails = useListDetails('album', listId);
  ...
  const getBody = () => {
    const firstSong = listDetails?.tracks.items[0].uri;
  ...
  return (
    <FeedCard
      imageSrc={listDetails?.images?.[1]?.url ?? ''}
      isPlaying={isPlaying}
